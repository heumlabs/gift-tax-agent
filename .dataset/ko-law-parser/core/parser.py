...
def parse_law(law_file):
    ...
    regex_article_title = re.compile(r'^(제[\d]+조(?:의[\d]+)*(?:\([^)]+\))?)')
        ...
        for line in lines:
            ...
            if pointer_contents == 1:  # index
                if regex_part.match(line): # start part
                    ...
                    match_article = regex_article_title.match(line)
                    if match_article:
                        key_article = match_article.group(1)
                    else:
                        key_article = line.split()[0]
                    if key_part:
                        if key_section:
                            if key_sub_section:
...
def get_article(law_file, article):
    ...
    regex_article_head = re.compile(r'^제[\d]+조')
    ...
    with open(law_file, 'r', encoding='utf-8') as f:
        ...
        for line in lines[1:]:
            ...
            if line == '':
                ...
                if not is_target:
                    continue
                # blank lines inside article: skip but keep collecting
                continue
            if not is_contents:
                if line == filename:
                    is_contents = True
                    continue
                if any(regex.match(line) for regex in regex_content_starts):
                    is_contents = True
                else:
                    continue
            ...
            if line[0] == "[":  # end of article
                break
            if regex_article_head.match(line) and not line.startswith(article):
                break
