...
**문서 버전**: v2.2
**작성일**: 2025-10-16
**최종 업데이트**: 2025-10-16 (Phase 3 상세 기획 반영)
**레퍼런스**: `05-gift-tax-calculator-spec.md` (계산 로직 및 변수 정의), `04-clarifying-implementation-spec.md` (Phase 3 구현 명세)
...
**총 9개 변수** (Phase 1에서 확정)

> **중요**: `past_gifts_value`, `past_tax_paid` 변수는 국세청 간편계산기 범위를 벗어나므로 Phase 1에서 제외되었습니다. 10년 합산 과세는 향후 확장 기능으로 고려됩니다.

### Tier 1: 필수 기본 정보 (우선 수집) - 3개
...
| 변수명 | 타입 | 기본값 | 설명 | Clarifying 질문 예시 |
|--------|------|--------|------|---------------------|
| `is_generation_skipping` | `boolean` | `false` | 세대생략 증여 여부 (조부모→손자) | "조부모님께서 손자/손녀에게 직접 증여하시나요? (부모님이 생존해 계신 경우)" |
| `is_minor_recipient` | `boolean` | `false` | 수증자 미성년자 여부 (만 19세 미만) | "증여받으시는 분이 미성년자(만 19세 미만)인가요?" |
| `is_non_resident` | `boolean` | `false` | 수증자 비거주자 여부 | "증여받으시는 분이 해외에 거주 중이신가요? (1년 중 183일 이상)" |

> **Phase 3 구현**: Tier 2 변수는 기본값 사용 가능. 명시적으로 질문하지 않고 기본값(false)으로 계산 진행. Phase 4에서 명시적 질문 추가 고려.

### Tier 3: 공제 및 채무 정보 - 3개

| 변수명 | 타입 | 필수 | 기본값 | 설명 | Clarifying 질문 예시 |
|--------|------|------|--------|------|---------------------|
| `marriage_deduction_amount` | `int` | 조건부 | `0` | 혼인공제액 (최대 1억) | "혼인 전후 2년 이내 증여받으신 것인가요?" |
| `childbirth_deduction_amount` | `int` | 조건부 | `0` | 출산공제액 (최대 1억) | "자녀 출생 2년 이내 증여받으신 것인가요?" |
| `secured_debt` | `int` | 선택 | `0` | 담보채무액 | "증여받은 재산에 담보대출이나 임대보증금이 있나요?" |

> **Phase 3 구현**: Tier 3 변수 중 `secured_debt`는 기본값 0 사용. 혼인/출산 공제는 `donor_relationship`이 직계존속/비속일 때만 질문.

### 질문 순서 원칙

1. **Tier 1 → Tier 2 → Tier 3** 순서로 질문
2. **Phase 3**: 한 번에 **정확히 1개** 질문만 생성 (사용자 경험 단순화)
3. **조건부 질문**: 해당 조건 충족 시에만 질문 (예: 직계존속/비속일 때만 혼인/출산 공제 질문)
4. **선택 질문**: 기본값으로 계산 가능하나, 정확도 향상을 위해 질문 권장 (Phase 4)
...
RELATIONSHIP_MAPPING = {
    # 사용자 친화적 표현 → enum 값 (수증자 기준)
    "배우자": "배우자",
    "부모": "직계존속",     # 부모→나 (내가 받음, 나 입장에서 부모는 직계존속)
    "부모님": "직계존속",
    "아버지": "직계존속",
    "어머니": "직계존속",
    "자녀": "직계비속",     # 나→자녀 (자녀가 받음, 자녀 입장에서 나는 직계존속이지만, 내가 세금 내므로 나 입장에서 자녀는 직계비속)
    "아들": "직계비속",
    "딸": "직계비속",
    "조부모": "직계존속",   # 조부모→나 (나 입장에서 조부모는 직계존속)
    "할아버지": "직계존속",
    "할머니": "직계존속",
    "손자": "직계비속",     # 나→손자 (나 입장에서 손자는 직계비속)
    "손녀": "직계비속",
    "형제": "기타친족",
    "자매": "기타친족",
    "친척": "기타친족",
...
> **주의**: LLM이 사용자 응답을 파싱할 때 위 매핑 테이블을 참조하여 정규화해야 합니다.
> **중요**: 증여세는 받는 사람(수증자)이 내므로, 수증자 입장에서의 관계를 입력합니다.
...
- LLM으로 초기 엔티티 추출
  ...
  - 주의: donor_relationship은 수증자 기준 (부모→자녀, 자녀 입장에서 부모 = 직계존속)
...
- 2025-10-16: v2.2 - Phase 3 상세 기획 반영 (9개 변수 명확화, past_gifts 제거 확정, 질문 개수 1개로 확정)
- 2025-10-15: v2.1 - 문서 정리
- 2025-10-15: v2.0 - 국세청 간편계산기 기준으로 전면 개편
- 2025-10-14: v1.0 - 초안 작성

## 관련 문서

- [04-clarifying-implementation-spec.md](./04-clarifying-implementation-spec.md) - Phase 3 구현 상세 명세 (파라미터 파싱, 질문 생성, Workflow 설계)
- [05-gift-tax-calculator-spec.md](./05-gift-tax-calculator-spec.md) - 9개 변수 및 계산 로직
- [03-message-format.md](./03-message-format.md) - Metadata 구조 정의
